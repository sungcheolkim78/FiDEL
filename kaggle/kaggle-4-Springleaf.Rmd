...
train <- train[, -c('ID', 'target1', 'VAR_1427', 'VAR_0847', 'VAR_1428')]

not_any_na <- function(x) all(!is.na(x))
train <- train %>% 
  select(where(not_any_na)) 

#train <- train[, c(sample(ncol(train)-1, 300), ncol(train)), with=F]
#head(train)
```
...
```{r}
model_list <- c('pls', 'rda', 'svmLinear', 'svmRadial', 'knn', 'earth', 'mlp', 'rf', 'gbm', 'ctree', 'C5.0', 'bayesglm', 'glm', 'glmnet', 'simpls', 'dwdRadial', 'xgbTree', 'xgbLinear', 'nnet')
t1 <- mtrainer(model_list, dataInfo = 'SpringLeaf')
# rpart is not working -> add gbm
# avNNet, nb is not working
