...
```{r}
y <- generate.labels(N=10000, rho=0.3)
table(y)
```

```{r}
gs <- generate.scores.gaussian(y, auc=0.8)
plot.scores(gs)
```

```{r}
lambda.auc(auc.rank(gs, y), N=attr(y, 'N'), rho=attr(y, 'rho'))
```

```{r}
data_g <- create_predictions(n=1000, m=10, p=0.3)
```
...
```{r}
fde1 <- fdensemble(data_g$predictions)
fde1 <- calculate_performance(fde1, data_g$actual_labels)
```
